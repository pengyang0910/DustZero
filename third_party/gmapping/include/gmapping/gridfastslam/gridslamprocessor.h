  ...
  */
   ...
   */
    ...
    */
    ...
    void initWithMap(unsigned int size, double xmin, double ymin, double xmax, double ymax, double delta,
    OrientedPoint initialPose=OrientedPoint(0,0,0));
    ...
    bool processScan(const RangeReading & reading, int adaptParticles=0,bool odomErr=false);
    bool initScan(const RangeReading & reading, int adaptParticles=0);
    void updateParticles(float angle);
    void updateParticles(OrientedPoint initialPose);
    void updateParticleScore(float score);
    bool rotateNode(TNode* n, float weight);
  ...
  private:
    ...
    inline void parallel(ScanMatcher &matcher, const double* plainReading, ParticleVector::iterator it, int ParticlesNum, double &sumScore);
