...
MotionModel::drawFromMotion(const OrientedPoint& p, const OrientedPoint& pnew, const OrientedPoint& pold) const{
	...
	OrientedPoint delta=absoluteDifference(pnew, pold);
	OrientedPoint noisypoint(delta);
	float dx= sampleGaussian(srr*fabs(delta.x)+str*fabs(delta.theta)+sxy*fabs(delta.y));
	float dy= sampleGaussian(srr*fabs(delta.y)+str*fabs(delta.theta)+sxy*fabs(delta.x));
	float dtheta= sampleGaussian(stt*fabs(delta.theta)+srt*sqrt(delta.x*delta.x+delta.y*delta.y));
	noisypoint.x+=dx;
	noisypoint.y+=dy;
	noisypoint.theta+=dtheta;
	noisypoint.theta=fmod(noisypoint.theta, 2*M_PI);
	if (noisypoint.theta>M_PI)
		noisypoint.theta-=2*M_PI;
	return absoluteSum(p,noisypoint);
...
OrientedPoint 
MotionModel::drawFromMotionNew(const OrientedPoint& p, const OrientedPoint& pnew, const OrientedPoint& pold) const{
	double sxy=0.3*srr;
	OrientedPoint delta=absoluteDifference(pnew, pold);
	delta.x=0.0050;
	delta.y=0.0050;
	delta.theta=0.005;
	OrientedPoint delta0;
	delta0.x=0;delta0.y=0;delta0.theta=0;
	OrientedPoint noisypoint(delta0);
	...
	// if(dtheta>0.2) dtheta=0.2;
	// else if(dtheta<-0.2) dtheta=-0.2;
	noisypoint.x+=dx;
	noisypoint.y+=dy;
	noisypoint.theta+=dtheta;
	noisypoint.theta=fmod(noisypoint.theta, 2*M_PI);
