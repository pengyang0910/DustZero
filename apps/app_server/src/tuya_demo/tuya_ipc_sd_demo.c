...
#if defined(SUPPORT_ALBUM) ||  defined(SUPPORT_ALBUM_EMERGECY) || defined(SUPPORT_ALBUM_PANORAMA)
    stg_var.album_info.cnt =2;
    memcpy(&stg_var.album_info.albumName[0], TUYA_IPC_ALBUM_EMERAGE_FILE,strlen(TUYA_IPC_ALBUM_EMERAGE_FILE));
    memcpy(&stg_var.album_info.albumName[1], "testAlbum",strlen("testAlbum"));
#endif
...
void album_jpg_file_put(char *jpgFile)
{
    ...
    sprintf(cmd, "cp %s/%s %s/%s", s_raw_emergency_path, jpgFile, s_emergency_path, targetFile);//TODO put jpg into the path
    system(cmd);
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "cp %s/%s %s/%s", s_raw_emergency_path, s_test_pic_thumbNail, s_emergency_thunbnail, targetFile);//TODO put thumbnail into the path
    system(cmd);
...
void album_mp4_file_put(char *mp4File)
{
    ...
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "cp %s/%s %s/%s", s_raw_emergency_path, mp4File, s_emergency_path, targetFile);//TODO put mp4 into the path
    printf("cmd:%s\n",cmd);
    system(cmd);

    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "cp %s/%s %s/%s", s_raw_emergency_path, s_test_mp4_thumbNail, s_emergency_thunbnail, mp4Pic);//TODO put thumbnail into the path
     printf("cmd:%s\n",cmd);
     system(cmd);
    // write file info
    ALBUM_FILE_INFO strInfo;
   ...
   // char* p = mp4File + strlen("1234567890_");
    strInfo.duration = 50;// todo check atoi(p);
    tuya_ipc_album_write_file_info(TUYA_IPC_ALBUM_EMERAGE_FILE, &strInfo);
...
#ifdef SUPPORT_ALBUM_PANORAMA
#define PanoramaFileName "Panorama.jpg"
void  album_file_put_Panorama()
{
    // write file info
    ALBUM_FILE_INFO strInfo;
    memset(&strInfo, 0, sizeof(ALBUM_FILE_INFO));
    strInfo.channel = 1;
    strInfo.type = SS_DATA_TYPE_Panorama;
    memcpy(&strInfo.fileName, PanoramaFileName, strlen(PanoramaFileName));
    strInfo.createTime = uni_time_get_posix();;
    strInfo.duration = 0;
    tuya_ipc_album_write_file_info(TUYA_IPC_ALBUM_EMERAGE_FILE, &strInfo);
