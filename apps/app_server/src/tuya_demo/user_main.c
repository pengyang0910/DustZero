...
#define IPC_APP_STORAGE_PATH    "/userdata/ipc_sweeper_robot/"   //Path to save tuya sdk DB files, should be readable, writeable and storable
#define IPC_APP_UPGRADE_FILE    "/userdata/upgrade.zip" //File with path to download file during OTA
#define IPC_APP_SD_BASE_PATH    "/tmp/"      //SD card mount directory
#else

#define IPC_APP_STORAGE_PATH    "/mnt/UDISK/fj212/resource/ipc_sweeper_robot/"   //Path to save tuya sdk DB files, should be readable, writeable and storable
#define IPC_APP_UPGRADE_FILE    "/mnt/UDISK/fj212/resource/upgrade.zip" //File with path to download file during OTA
#define IPC_APP_SD_BASE_PATH    "/tmp/"      //SD card mount directory
#endif
...
//#define IPC_APP_VERSION         "2.14.3"     //Firmware version displayed on TUYA APP

pthread_mutex_t dp_sdk_data_mutex = PTHREAD_MUTEX_INITIALIZER;
int lcmSeq = 0;
int16_t originPx = 0;
int16_t originPy = 0;
int mapWidth = 0;
...
CHAR_T s_sdk_version[64]="";
CHAR_T s_ipc_pid[64]= "gjjfkamnyxby2xiz";//car Product ID of TUYA device, this is for demo only.
//CHAR_T s_ipc_uuid[64]="tuya2defb5a0b271ab2a";//Unique identification of each device//Contact tuya PM/BD for developing devices or BUY more
//CHAR_T s_ipc_authkey[64]="xA31xhNFhlN66Bl76rPfkcxfypJu33Tg";//Authentication codes corresponding to UUID, one machine one code, paired with UUID.
CHAR_T s_ipc_uuid[64]="";//Unique identification of each device//Contact tuya PM/BD for developing devices or BUY more
CHAR_T s_ipc_authkey[64]="";//Authentication codes corresponding to UUID, one machine one code, paired with UUID.
...
{
    printf("tuya icp start \n");
    
    PR_DEBUG("SDK Version:%s\r\n", tuya_ipc_get_sdk_info());

    const char *dir = "/userdata/ipc_sweeper_robot/";
#ifdef RK3566_BUILD
    if (access(dir,0)==-1)
    {
         mkdir(dir);
	...
	strcpy(ipc_sdk_run_var.iot_info.dev_sw_version,s_sdk_version);
    ...
    ipc_sdk_run_var.media_info.media_info.channel_enable[E_CHANNEL_VIDEO_MAIN] = TRUE;    /* Whether to enable local HD video streaming */
    ipc_sdk_run_var.media_info.media_info.video_fps[E_CHANNEL_VIDEO_MAIN] = 30;  /* FPS */
    ipc_sdk_run_var.media_info.media_info.video_gop[E_CHANNEL_VIDEO_MAIN] = 30;  /* GOP */
    ipc_sdk_run_var.media_info.media_info.video_bitrate[E_CHANNEL_VIDEO_MAIN] = TUYA_VIDEO_BITRATE_1M; /* Rate limit */
    ipc_sdk_run_var.media_info.media_info.video_width[E_CHANNEL_VIDEO_MAIN] = 640; /* Single frame resolution of width*/
    ipc_sdk_run_var.media_info.media_info.video_height[E_CHANNEL_VIDEO_MAIN] = 360;/* Single frame resolution of height */
    ipc_sdk_run_var.media_info.media_info.video_freq[E_CHANNEL_VIDEO_MAIN] = 90000; /* Clock frequency */
    ...
    ipc_sdk_run_var.media_info.media_info.channel_enable[E_CHANNEL_VIDEO_SUB] = TRUE;     /* Whether to enable local SD video stream */
    ipc_sdk_run_var.media_info.media_info.video_fps[E_CHANNEL_VIDEO_SUB] = 30;  /* FPS */
    ipc_sdk_run_var.media_info.media_info.video_gop[E_CHANNEL_VIDEO_SUB] = 30;  /* GOP */
    ipc_sdk_run_var.media_info.media_info.video_bitrate[E_CHANNEL_VIDEO_SUB] = TUYA_VIDEO_BITRATE_512K; /* Rate limit */
    ipc_sdk_run_var.media_info.media_info.video_width[E_CHANNEL_VIDEO_SUB] = 640; /* Single frame resolution of width */
    ipc_sdk_run_var.media_info.media_info.video_height[E_CHANNEL_VIDEO_SUB] = 360;/* Single frame resolution of height */
    ipc_sdk_run_var.media_info.media_info.video_freq[E_CHANNEL_VIDEO_SUB] = 90000; /* Clock frequency */
    ipc_sdk_run_var.media_info.media_info.video_codec[E_CHANNEL_VIDEO_SUB] = TUYA_CODEC_VIDEO_H264; /* Encoding format */

    /* Audio stream configuration.
    Note: The internal P2P preview, cloud storage, and local storage of the SDK are all use E_CHANNEL_AUDIO data. */
    ipc_sdk_run_var.media_info.media_info.channel_enable[E_CHANNEL_AUDIO] = TRUE;         /* Whether to enable local sound collection */
    ipc_sdk_run_var.media_info.media_info.audio_codec[E_CHANNEL_AUDIO] = TUYA_CODEC_AUDIO_PCM;/* Encoding format */
    ipc_sdk_run_var.media_info.media_info.audio_sample [E_CHANNEL_AUDIO]= TUYA_AUDIO_SAMPLE_8K;/* Sampling Rate */
    ipc_sdk_run_var.media_info.media_info.audio_databits [E_CHANNEL_AUDIO]= TUYA_AUDIO_DATABITS_16;/* Bit width */
    ipc_sdk_run_var.media_info.media_info.audio_channel[E_CHANNEL_AUDIO]= TUYA_AUDIO_CHANNEL_MONO;/* channel */
    ipc_sdk_run_var.media_info.media_info.audio_fps[E_CHANNEL_AUDIO] = 25;/* Fragments per second */
	...
	ipc_sdk_run_var.p2p_info.transfer_event_cb = (TRANSFER_EVENT_CB)__TUYA_APP_p2p_event_cb;
	...
	ipc_sdk_run_var.dp_info.dp_query = (TUYA_IPC_SDK_DP_QUERY)IPC_APP_handle_dp_query_objs;
	ipc_sdk_run_var.dp_info.raw_dp_cmd_proc = (TUYA_IPC_SDK_RAW_DP_CMD_PROC)IPC_APP_handle_raw_dp_cmd_objs;
	ipc_sdk_run_var.dp_info.common_dp_cmd_proc = (TUYA_IPC_SDK_COMMON_DP_CMD_PROC)IPC_APP_handle_dp_cmd_objs;

	/*upgrade function(essential)*/
	ipc_sdk_run_var.upgrade_info.enable = true;
	ipc_sdk_run_var.upgrade_info.upgrade_cb = (TUYA_IPC_SDK_DEV_UPGRADE_INFORM_CB)IPC_APP_Upgrade_Inform_cb;
    ...
    if(ret !=0 )
    {
        ...
        #else
            system("/mnt/UDISK/fj212/restartApp.sh");
        #endif
    }

    //初始化扫地机SDK
    tuya_sweeper_init_sdk();
...
extern int g_file_arr[SWEEPER_ALBUM_FILE_ALL_TYPE_COUNT];
...

{
...
#ifdef RK3566_BUILD
    strcpy(s_raw_path, "/app/fj212"); //Path where demo resources locates
#else 
    strcpy(s_raw_path, "/mnt/UDISK/fj212"); //Path where demo resources locates
#endif
#ifdef __HuaweiLite__
    if(argc != 2)
    {
    ...
    while(1)
    {
        ...
        extern int g_support_file;
        g_support_file = 1;
        
        // 休眠时间表示如果传输流文件，多长时间再次传输（如果只传输文件，则该参数不生效）
        extern int g_sleep_time;
        g_sleep_time = 1;
        
        // 输入各种文件的路径,顺序如之前所示(map.bin cleanPath.bin, navPath.bin)
    
        extern char g_file_path_name[3][256];
        memcpy(g_file_path_name[0], "/tmp/map.bin.stream", 256);
        memcpy(g_file_path_name[1], "/tmp/cleanPath.bin.stream", 256);
        memcpy(g_file_path_name[2], "/tmp/navPath.bin1", 256);  
...
{
    snprintf(s_ipc_pid, 64,"%s", data);
}

void set_s_ipc_uuid(char data[64])
{
    snprintf(s_ipc_uuid, 64,"%s", data);
}

void set_s_ipc_authkey(char data[64])
{
    snprintf(s_ipc_authkey, 64,"%s", data);
}

void set_s_sdk_version(char data[64])
{
    snprintf(s_sdk_version, 64,"%s", data);
}
...
TY_RECV_OBJ_DP_S * sdk_dp_cmd_data;
void CmdDataSet(TY_RECV_OBJ_DP_S * data)
{
    pthread_mutex_lock(&dp_sdk_data_mutex);
    sdk_dp_cmd_data = data;
    pthread_mutex_unlock(&dp_sdk_data_mutex);
}

TY_RECV_OBJ_DP_S * CmdDataGet()
{
    pthread_mutex_lock(&dp_sdk_data_mutex);
    TY_RECV_OBJ_DP_S * rn = sdk_dp_cmd_data;
    pthread_mutex_unlock(&dp_sdk_data_mutex);
    return rn;
}
...
TY_RECV_RAW_DP_S * sdk_dp_raw_data;
void RawDataSet(TY_RECV_RAW_DP_S * data)
{
    pthread_mutex_lock(&dp_sdk_data_mutex);
    sdk_dp_raw_data= data;
    pthread_mutex_unlock(&dp_sdk_data_mutex);
}
...
{
    pthread_mutex_lock(&dp_sdk_data_mutex);
    TY_RECV_RAW_DP_S * rn = sdk_dp_raw_data;
    pthread_mutex_unlock(&dp_sdk_data_mutex);
    return rn;
}
