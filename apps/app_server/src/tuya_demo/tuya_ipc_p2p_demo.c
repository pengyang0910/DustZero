...
INT_T __TUYA_APP_p2p_event_cb(IN CONST TRANSFER_EVENT_E event, IN CONST PVOID_T args)
{
    ...
    switch (event)
    {
        ...
        case TRANS_DOWNLOAD_START:
        {
            ...
            strParm.pFileInfoArr = (SS_FILE_INFO_S*)malloc(pParam->fileNum*sizeof(SS_FILE_INFO_S));
            ...
            if(pParam->pFileInfo)
            {
                strParm.pFileInfoArr->start_timestamp = pParam->pFileInfo->start_timestamp;
                strParm.pFileInfoArr->end_timestamp = pParam->pFileInfo->end_timestamp;
            }
            else
            {
        ...
        case TRANS_DOWNLOAD_IMAGE:
        {
            ...
            printf("TRANS_DOWNLOAD_IMAGE\n");
            if (OPRT_OK !=  tuya_ipc_ss_download_image(pParam->channel, strParm,
                                    &(pParam->image_fileLength), (char **)&(pParam->pBuffer)))
            {
                printf("tuya_ipc_download_image err \n");
            }
            break;
        }
        case TRANS_PLAYBACK_DELETE:
        {
            ...
            if (OPRT_OK !=  tuya_ipc_delete_video(pParam->channel, pParam->year,pParam->month, pParam->day))
            {
                printf("tuya_ipc_ss_delete_video err\n");
            }
            break;
        }
        #endif
        ...
        case TRANS_ALBUM_QUERY: /* query album */
        {
            ...
            printf("p2p map TRANS_ALBUM_QUERY, albumName:%s\n", pSrcType->albumName);
