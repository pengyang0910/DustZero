...
#ifdef TUYA_DP_SWEEPER_POWER_ON
STATIC VOID handle_DP_POWER_ON(IN TY_OBJ_DP_S *p_obj_dp)
{
    BOOL_T power_mode = check_dp_bool_invalid(p_obj_dp);
    printf("handle_DP_POWER_ON %d \r\n", power_mode);

 //   IPC_APP_set_sleep_mode(power_mode);
 //   power_mode = IPC_APP_get_sleep_mode();

    respone_dp_bool(TUYA_DP_SWEEPER_POWER_ON, power_mode);
}
#endif
...
{
#ifdef TUYA_DP_SWEEPER_POWER_ON
     {TUYA_DP_SWEEPER_POWER_ON,     handle_DP_POWER_ON},
#endif
#ifdef TUYA_DP_SLEEP_MODE
...
VOID IPC_APP_handle_dp_cmd_objs(IN CONST TY_RECV_OBJ_DP_S *dp_rev)
{
    ...
    for(index = 0; index < cnt; index++)
    {
        ...
        for(table_idx = 0; table_idx < table_count; table_idx++)
        {
            ...
            {
                s_dp_table[table_idx].handler = (TUYA_DP_HANDLER)p_dp_obj;
                break;
            }
        }
    }
...
VOID IPC_APP_handle_raw_dp_cmd_objs(IN CONST TY_RECV_RAW_DP_S *dp_rev)
{
    ...
    for(table_idx = 0; table_idx < table_count; table_idx++)
    {
        ...
        {
            s_raw_dp_table[table_idx].handler = (TUYA_RAW_DP_HANDLER)dp_rev;
            break;
        }
    }
}
