...
typedef enum
{
    E_RINGBUFFER_STREAM_V_MAIN ,
    E_RINGBUFFER_STREAM_V_SUB  ,
    E_RINGBUFFER_STREAM_V_3RD  ,
    E_RINGBUFFER_STREAM_V_4TH  ,
    E_RINGBUFFER_STREAM_V_MAX  = 8,
    E_RINGBUFFER_STREAM_A  ,
    E_RINGBUFFER_STREAM_A_2RD ,
    E_RINGBUFFER_STREAM_A_3RD ,
    E_RINGBUFFER_STREAM_A_4TH ,
    E_RINGBUFFER_STREAM_MAX  = 16,
} RINGBUFFER_STREAM_E;
...
typedef VOID (*FUNC_REQUEST_KEY_FRAME)(INT_T device, INT_T channel, RINGBUFFER_STREAM_E stream);
...
*/
OPERATE_RET tuya_ipc_ring_buffer_init(INT_T device, INT_T channel, RINGBUFFER_STREAM_E stream, Ring_Buffer_Init_Param_S* pparam);

/* uninitialize one ring buffer for one channel(one device)
device: ring buffer device num, multipul device for one 
*/
OPERATE_RET tuya_ipc_ring_buffer_uninit(INT_T device, INT_T channel, RINGBUFFER_STREAM_E stream);
...
OPERATE_RET tuya_ipc_ring_buffer_destory(CHANNEL_E device);
#endif

int tuya_ipc_ring_buffer_get_video_num_skill(INT_T device, INT_T channel);
/* open a new session for read/write
*/
Ring_Buffer_User_Handle_S tuya_ipc_ring_buffer_open(INT_T device, INT_T channel, RINGBUFFER_STREAM_E stream, RBUF_OPEN_TYPE_E open_type);
/* close session
*/
...
*/
INT_T tuya_ipc_ring_buffer_get_max_buf_secs(int device, int channel, RINGBUFFER_STREAM_E stream);

/* set max frame size of device
*/
INT_T tuya_ipc_ring_buffer_set_max_frame_size(INT_T device, INT_T channel, RINGBUFFER_STREAM_E stream, UINT_T maxFrameSize);

/* get max frame size, return main channel size
*/
INT_T tuya_ipc_ring_buffer_get_max_frame_size(INT_T device, INT_T channel, RINGBUFFER_STREAM_E stream);
/*
