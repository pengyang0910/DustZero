...
void pf_kdtree_insert(pf_kdtree_t *self, pf_vector_t pose, double value)
{
  ...
  // }
  // printf("2:key[2]=%d \n",key[2]);
  // printf("size x:%f,y:%f,theta:%f,pose x:%f,y:%f,theta:%f,key x:%d,y:%d,theta:%d \n",
  //         self->size[0],self->size[1],self->size[2],pose.v[0],pose.v[1],pose.v[2],
  //         key[0],key[1],key[2]);
...
pf_kdtree_node_t *pf_kdtree_insert_node(pf_kdtree_t *self, pf_kdtree_node_t *parent,
                                        pf_kdtree_node_t *node, int key[], double value)
{
  static int insertdepth=0;
  static int maxinsertdepth=0;
  insertdepth++;
  ...
  if (node == NULL)
  {
    //printf("node==null,insert depth:%d \n",insertdepth);
    if(insertdepth > maxinsertdepth)
    {
      maxinsertdepth=insertdepth;
    }  
    //printf("max insert depth:%d \n",maxinsertdepth);  
    insertdepth=0;
  ...
  else
  {
    ...
    loopCount++;
    //printf("insert node loop count:%d \n",loopCount);
...
void pf_kdtree_cluster_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int depth)
{
  ...
  for (i = 0; i < 3 * 3 * 3; i++)
  {
    ...
    //printf("search next depth:%d \n",depth+1);
    if(depth+1 > maxDepth)
    {
      maxDepth=depth+1;
      //printf("max depth:%d \n",maxDepth);
    }
    //printf("next node key,depth:%d x:%d,y;%d,theta:%d \n",depth+1,nkey[0],nkey[1],nkey[2]);
