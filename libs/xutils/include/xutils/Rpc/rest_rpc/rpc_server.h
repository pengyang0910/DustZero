...
public:
  rpc_server(unsigned short port, size_t size, size_t timeout_seconds = 15,
      ...
      : io_service_pool_(std::thread::hardware_concurrency()), acceptor_(io_service_pool_.get_io_service(),
                                          tcp::endpoint(tcp::v4(), port)),
        timeout_seconds_(timeout_seconds), check_seconds_(check_seconds),
        signals_(io_service_pool_.get_io_service()) {
    do_accept();
    check_thread_ = std::make_shared<std::thread>([this] { bindCpuCore(BIND_CPU_ID_RPC); prctl(PR_SET_NAME, "RpcCheck"); clean(); });
    pub_sub_thread_ =
        std::make_shared<std::thread>([this] { bindCpuCore(BIND_CPU_ID_RPC); prctl(PR_SET_NAME, "RpcPub"); clean_sub_pub(); });
    //signals_.add(SIGINT);
  ...
  rpc_server(unsigned short port, size_t size, ssl_configure ssl_conf,
      ...
      : rpc_server(port, std::thread::hardware_concurrency(), timeout_seconds, check_seconds) {
  ...
  void async_run() {
    thd_ = std::make_shared<std::thread>([this] { bindCpuCore(BIND_CPU_ID_RPC); prctl(PR_SET_NAME, "RpcAsyncRun"); io_service_pool_.run(); });
  }

  void run() { bindCpuCore(BIND_CPU_ID_RPC); prctl(PR_SET_NAME, "RpcRun");  io_service_pool_.run(); }
