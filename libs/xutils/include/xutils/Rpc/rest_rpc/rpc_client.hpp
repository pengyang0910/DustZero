...
public:
  ...
  rpc_client(client_language_t client_language,
      ...
      : socket_(ios_), work_(ios_), host_(std::move(host)), port_(port),
        deadline_(ios_), body_(INIT_BUF_SIZE), client_language_(client_language),
        on_result_received_callback_(std::move(on_result_received_callback)) {
    thd_ = std::make_shared<std::thread>([this] { 
		bindCpuCore(BIND_CPU_ID_RPC);
		prctl(PR_SET_NAME, "RpcClient3"); 
		ios_.run(); });
  }
  ...
  bool wait_conn(size_t timeout) {
    ...
    conn_cond_.wait_for(lock, std::chrono::seconds(timeout),
