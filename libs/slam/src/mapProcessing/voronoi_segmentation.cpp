		...
		double min_critical_point_distance_factor, double max_area_for_merging, bool display_map)
{
	...
	cv::distanceTransform(map_to_be_labeled, distance_map, 2, 5);
...
//	cv::erode(voronoi_map_, voronoi_map_, cv::Mat(), cv::Point(-1, -1), 1);
	...
	cv::Mat img_room = cv::Mat(cv::Size(segmented_map.cols, segmented_map.rows), CV_8UC3, cv::Scalar(0));;
	for (int i = 0;i < rooms.size();i++)
	{
		const cv::Vec3b color((rand() % 250) + 1, (rand() % 250) + 1, (rand() % 250) + 1);
		std::vector<cv::Point> pts = rooms[i].getMembers();
		for (int j = 0;j < rooms[i].getMembers().size();j++)
		{
			img_room.at<cv::Vec3b>(pts[j].y, pts[j].x) = color;
		}
	}
