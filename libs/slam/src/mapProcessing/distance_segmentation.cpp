...
void DistanceSegmentation::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map, double map_resolution_from_subscription, double room_area_factor_lower_limit, double room_area_factor_upper_limit)
{
	...
	cv::distanceTransform(temporary_map, distance_map, 2, 5);
	...
	//for (int current_threshold = 255; current_threshold > 7; current_threshold--)
	{ //change the threshold for the grayscale-image from largest possible value to smallest
	  //reset number of rooms
		...
		//if (temporary_contours.size() >0/* &&temporary_contours.size() >= saved_contours.size() */)//(current_energy >= saved_energy)
		if (temporary_contours.size() >0 &&temporary_contours.size() >= saved_contours.size())//(current_energy >= saved_energy)
		{
			...
			printf("current_threshold is %d  %d \n",current_threshold,saved_contours.size());
		}
	}

	if (temporary_contours.size() ==0)
	{
		for (int current_threshold = 0; current_threshold > 0; current_threshold--)
		{ //change the threshold for the grayscale-image from largest possible value to smallest
		//reset number of rooms
			temporary_contours.clear();
			contours.clear();
	...
	for (int current_contour = 0; current_contour < saved_contours.size(); current_contour++)
	{
		...
		do
		{
			...
			if (!contains(already_used_colors, fill_colour) || loop_counter > 250)
			{
				...
				//cv::drawContours(segmented_map, saved_contours, current_contour, fill_colour, 3);
