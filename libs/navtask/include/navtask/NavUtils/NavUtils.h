...
enum class OrientationState { Left = 0, Right = 1, Middle = 2 };
...
enum class CleaningState { Normal, TrackLine, ObsByPassRtn, CleanPatch, Mute };
...
typedef struct NavPoint {
  ...
  double norm() { return sqrt(pow(px, 2) + pow(py, 2)); }
  ...
  double DotProduct(const NavPoint &p) { return px * p.px + py * p.py; }
...
typedef struct LaserData {
  ...
  LaserData(float range_p, float bearing_p, int index_p)
      : range(range_p), bearing(bearing_p), index(index_p) {}
...
typedef struct LineData {
  ...
  LineData(float dis_p, float dis_mid, float lens_p, float angle_p,
           NavPoint &start, NavPoint &end, NavPoint &proj)
      : distance(dis_p), dis2mid(dis_mid), length(lens_p), local_angle(angle_p),
        start_point(start), end_point(end), proj_point(proj){};
...
enum class NavStatus { Normal = 0, Error };
...
enum class MapStatus { Occupied = 100, Unknow = -1, Free = 0 };
...
enum class BumperFlag { None_bumper, Left_bumper, Right_bumper, Both_bumper };
...
/**/
inline float dtor(float deg) { return static_cast<float>(deg * M_PI / 180.0); }
inline float rtod(float rad) { return static_cast<float>(rad * 180.0 / M_PI); }
...
typedef struct Segment {
  ...
  void reset() { Segment(); }
  void setConstructForbidden(bool forbidden) { constructForbidden = forbidden; }
  float getSegmentLen() {
...
float pid_update(pid_controller_t *pPID, float target, float fbk,
                 float ki_adjust_factor = 1.0f);
void pid_init(pid_controller_t *pPID, float KP, float KI, float KD,
              float maxOutput, float minError);
float pid_update_with_D_window(pid_controller_t *pPID, float target, float fbk,
                               float ki_adjust_factor = 1.0f, int D_window = 5);
