...
bool XLog::Initialise( const std::string& fileName ) {
	...
	if( !m_initialised ) {
		...
		m_initialised = true;
		m_stream<<"\n"<<std::endl;
		Info( "*******************  %s LOG INITIALISED  *********************", fileName.c_str() );
		return true;
	}
	else
	{
...
bool XLog::Finalise() {
	...
	if( m_initialised ) {
		Info( "*******************  %s LOG FINALISED  *********************", m_fileName.c_str()  );
		m_stream.close();
		m_stream<<"\n"<<std::endl;
		return true;
	}
 ...
 */
void XLog::SetThreshold( const Type& type ) {
	m_threshold = type;
}
...
bool XLog::log( const Type& type, const std::string& message ) {	
	if( type <= m_threshold ) {
		...
		this->write( "[%s] %s - %s", buffer, TypeToString( type ), message.c_str() );
		return true;
	}
	return false;
}
 ...
 * @return  True if the log was successful
 */
bool XLog::log( const Type& type, const char* format, const va_list& varArgs) {
	char buffer[512];
	vsnprintf( buffer, sizeof(buffer), format, varArgs);
