...
namespace ini
{
    ...
    inline void ShowTraceStack(const char* szBriefInfo)
    {
        //printf(">>>>>>>>> ShowTraceStack <<<<<<<<<<<<<\r\n");
        FILE* stream = fopen("/tmp/cfgErr.log", "a+");

        fprintf(stream, "%s", szBriefInfo);
        fflush(stream);
        fclose(stream);
    }
    ...
    struct Convert<bool>
    {
        void decode(const std::string& value, bool& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to bool!!!!!" << std::endl;
                //ShowTraceStack(ss.str().c_str());
    ...
    struct Convert<short>
    {
        void decode(const std::string& value, short& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to short!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                throw std::invalid_argument(ss.str().c_str());
                exit(-1);
            }
    ...
    struct Convert<unsigned short>
    {
        void decode(const std::string& value, unsigned short& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to unsigned short!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                throw std::invalid_argument(ss.str().c_str());
                exit(-1);
            }
    ...
    struct Convert<int>
    {
        void decode(const std::string& value, int& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to long!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                throw std::invalid_argument(ss.str().c_str());
                exit(-1);
            }
    ...
    struct Convert<unsigned int>
    {
        void decode(const std::string& value, unsigned int& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to unsigned int!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                throw std::invalid_argument(ss.str().c_str());
                exit(-1);
            }
    ...
    struct Convert<unsigned long>
    {
        void decode(const std::string& value, unsigned long& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to unsigned long!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                throw std::invalid_argument(ss.str().c_str());
                exit(-1);
            }
    ...
    struct Convert<double>
    {
        void decode(const std::string& value, double& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to double!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                //printf("222\r\n");
    ...
    struct Convert<float>
    {
        void decode(const std::string& value, float& result)
        {
            ...
            {
                std::stringstream ss;
                ss << "can not convert [" << value << "] to float!!!!!" << std::endl;
                ShowTraceStack(ss.str().c_str());
                throw std::invalid_argument(ss.str().c_str());
                exit(-1);
            }
    ...
    public:
        IniField() : value_()
        {
            //printf("IniField 1111111111\r\n");
        }

        IniField(const std::string& value) : value_(value)
        {
            //printf("IniField 222222 value: %s\r\n", value_.c_str());
        }
        IniField(const IniField& field) : value_(field.value_)
        {
            //printf("IniField 333333 value: %s\r\n", value_.c_str());
        }

        void SetLineNo(int n, std::string key)
        {
            lineNo = n;
            key_ = key;
        }
        ...
        T as() const
        {
            ...
            {
                std::stringstream ss;
                ss << "ERROR: Value empty!!!." << "\r\n";

                ShowTraceStack(ss.str().c_str());
                //throw std::logic_error(ss.str());
                exit(-1);
                //system("killall navClean appTask slam localization");
            }
            ...
            catch (const std::exception& e)
            {
                std::stringstream ss;
                ss << "line." << lineNo
                    << ": decode ERROR! key=" << key_ << " value=" << value_ \
                    << "! " << e.what() << "\r\n";
    ...
    private:
        ...
        std::map<std::string, std::string> key2valuePairs;
        std::map<std::string, std::map<std::string, std::string>> sec2keyPairs;
    ...
    public:
        ...
        IniField GetProperty(std::string secName_, std::string keyName_)
        {

            std::stringstream ss;
            if (sec2keyPairs.find(secName_) != sec2keyPairs.end())
            {
                if (sec2keyPairs[secName_].find(keyName_) != sec2keyPairs[secName_].end())
                {
                    return IniField(sec2keyPairs[secName_][keyName_]);
                }
                else
                {

                    ss << "ERROR: " << "Can't find key[" << keyName_ << "] in [" << secName_ << "]!!" << " \r\n";
                    ShowTraceStack(ss.str().c_str());
            ...
            {
                ss << "ERROR: " << "Can't find Section[" << secName_ << "]!" << " \r\n";
                ShowTraceStack(ss.str().c_str());
            }
        ...
        std::string GetKey(std::string key)
        {
            //printf("GetKey here!\r\n");
            std::string ret = key;
            if (key2valuePairs.find(key) == key2valuePairs.end())
            {
                std::stringstream ss;
                ss << "ERROR: " << "Can't find key[" << key << "]!" << " \r\n";
                ShowTraceStack(ss.str().c_str());
                throw std::logic_error(ss.str());
                exit(-1);
            }
        ...
        void decode(std::istream& is)
        {
            ...
            std::string line;
            std::map<std::string, std::string> tmpKeyValueMapPtr;
            std::string secName;
            ...
            while (!is.eof() && !is.fail())
            {
                ...
                if (line[0] == '[')
                {
                    ...
                    sec2keyPairs[secName] = tmpKeyValueMapPtr;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            printf("line_%d: name: %s value: %s\r\n", lineNo, name.c_str(), value.c_str());
                            std::stringstream ss;
                            ss << "line." << lineNo
                                << ": ini parsing failed, no value"
                                << " found for key ["
                                << name << "]\r\n";
                            throw std::logic_error(ss.str());
                            exit(-1);
                        ...
                        //printf("************ 222222222 value %s\r\n", value.c_str());
                        key2valuePairs[name] = value;
                        sec2keyPairs[secName][name] = value;
                        (*currentSection)[name] = value;
                        (*currentSection)[name].SetLineNo(lineNo, name);
                        // auto tmp = (*currentSection)[name];
